...
<div class="mb-8">
    <h1 class="text-2xl font-bold text-slate-900">Editar despesa</h1>
    <p class="text-sm text-slate-500 mt-1">Atualize as informações desta despesa.</p>
</div>
    ...
    <form method="POST" action="{{ route('expenses.update', $expense) }}">
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-6 mb-4 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="description" value="{{ old('description', $expense->description) }}" autofocus
        ...
        <div class="bg-slate-50 rounded-xl border border-slate-200 p-4 mb-4 text-xs text-slate-500 flex items-center justify-between">
            <span>Registrada em {{ $expense->created_at->format('d/m/Y') }}</span>
            <span>Atualizada {{ $expense->updated_at->diffForHumans() }}</span>
        </div>
